...
// the master-BRD error envelope and pagination, Idempotency-Key replay and
// the resource handlers. Contracts are identical to identity-service and
// rbac-service (CONVENTIONS: wave-1 vendored helpers).
package api
...
type Claims struct {
	...
	WorkspaceID  string   `json:"workspace_id,omitempty"` // request workspace; required for workspace-scoped authz
...
// EffectiveUser is whose permissions and fairness slots apply (OBO →
// original user, MASTER-FR-015).
func (c *Claims) EffectiveUser() string {
	if c.Typ == domain.TypAgentOBO && c.OboSub != "" {
		return c.OboSub
	}
	return c.Sub
}

// Verifier validates RS256 JWTs (MASTER-FR-010): signature via JWKS
// (cached, refresh ≤ 5 min) or a static test key; exp/iss/aud validated.
// alg=none and non-RS256 algorithms are rejected outright (MASTER-FR-014).
...
func (v *Verifier) Verify(ctx context.Context, tokenString string) (*Claims, error) {
	...
	opts := []jwt.ParserOption{
		jwt.WithValidMethods([]string{"RS256"}), // alg=none forbidden
