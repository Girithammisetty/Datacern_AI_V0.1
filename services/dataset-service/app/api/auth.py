...
- Internal requests: SPIFFE identity header injected by the mesh sidecar after
  mTLS termination (MASTER-FR-014); the profiler callback additionally carries
  an HMAC body signature bound to a per-job single-use token (DST-FR-023).
...
class Principal:
    ...
    def effective_user(self) -> str:
        ...
        every agent_obo grounding call (e.g. the inference agent's dataset reads)."""
