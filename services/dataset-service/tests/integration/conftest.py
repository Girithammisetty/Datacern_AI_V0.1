...
non-privileged RLS role. Auto-skips with a clear message when Docker is down."""
...
def pg():
    ...
    os.environ["DST_MIGRATE_URL"] = super_url.replace("postgresql://", "postgresql+psycopg://")
    ...
    with psycopg.connect(super_url, autocommit=True) as conn:
        conn.execute(
            f"CREATE USER {APP_USER} LOGIN PASSWORD '{APP_PASSWORD}' IN ROLE dataset_app"
        )
...
async def container(engine, tmp_path, clock):
    ...
    return build_container(
        make_settings(tmp_path), mode="sql", session_factory=session_factory, clock=clock
    )
