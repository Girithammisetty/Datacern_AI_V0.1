...
# Rows appended by the second ingestion only (delta mode).
DF_DELTA = pd.DataFrame(
    {
        "claim_id": ["C90", "C91", "C92"],
        "status": ["open", "denied", "open"],
        "vendor": ["initech", "initech", "acme"],
        "amount": [700, 15, 305],
    }
)


class TestBrowseRowsIngestionDelta:
    """delta_ingestion_id: the case-trigger applier's incremental read — only
    the rows appended by one ingestion, valid BEFORE the version registers."""

    async def test_delta_returns_only_the_new_ingestions_rows_unregistered(
        self, client, container
    ):
        ds = await _seed(client, container)  # snapshot 7001, registered
        # Second ingestion committed to the catalog but with NO version
        # registration — exactly the window where the legacy full browse races.
        await container.catalog.commit_snapshot(
            ds["iceberg_table"], 7002, DF_DELTA, summary={"ingestion_id": "ing-b"}
        )
        data = await _rows(client, ds["id"], delta_ingestion_id="ing-b")
        assert data["total"] == 3
        assert data["filtered"] == 3
        assert {r[0] for r in data["rows"]} == {"C90", "C91", "C92"}

    async def test_delta_filters_apply_within_the_delta_only(self, client, container):
        ds = await _seed(client, container)
        await container.catalog.commit_snapshot(
            ds["iceberg_table"], 7002, DF_DELTA, summary={"ingestion_id": "ing-b"}
        )
        # 'open' matches 5 rows in the base snapshot but only 2 in the delta.
        data = await _rows(
            client, ds["id"], delta_ingestion_id="ing-b", filter="status:eq:open"
        )
        assert data["total"] == 3
        assert data["filtered"] == 2
        assert {r[0] for r in data["rows"]} == {"C90", "C92"}

    async def test_delta_unknown_ingestion_is_an_empty_page(self, client, container):
        ds = await _seed(client, container)
        data = await _rows(client, ds["id"], delta_ingestion_id="ing-nope")
        assert data["total"] == 0
        assert data["rows"] == []
        assert data["columns"] == []

    async def test_delta_spans_chunked_snapshots_of_one_ingestion(
        self, client, container
    ):
        # One large ingestion commits several chunk snapshots, all stamped with
        # the same ingestion_id — the delta is their union, not the last chunk.
        ds = await _seed(client, container)
        chunk1 = DF_DELTA.iloc[:2]
        chunk2 = DF_DELTA.iloc[2:]
        await container.catalog.commit_snapshot(
            ds["iceberg_table"], 7002, chunk1, summary={"ingestion_id": "ing-b"}
        )
        await container.catalog.commit_snapshot(
            ds["iceberg_table"], 7003, chunk2, summary={"ingestion_id": "ing-b"}
        )
        data = await _rows(client, ds["id"], delta_ingestion_id="ing-b")
        assert data["total"] == 3
        assert {r[0] for r in data["rows"]} == {"C90", "C91", "C92"}
