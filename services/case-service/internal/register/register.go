...
// (RBC-FR-022): case-service pushes its action manifest to rbac's idempotent
...
// Register mints a service token and POSTs case-service's action manifest to
...
func Register(ctx context.Context, cfg Config) error {
	...
	// Retry a few times: rbac may still be coming up when case-service boots.
	var lastErr error
...
func mintServiceToken(key *rsa.PrivateKey, cfg Config) (string, error) {
	...
	claims := jwt.MapClaims{
		...
		"jti":       fmt.Sprintf("case-register-%d", now.UnixNano()),
	}
