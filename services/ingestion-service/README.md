...
Datacern ingestion-service (BRD 03): source **connections**, **ingestion jobs**
...
Dev configuration is via env vars (`DATABASE_URL`, `DATACERN_ENV`,
`DATACERN_DATA_DIR`, `JWT_PUBLIC_KEY_PEM`, `JWT_ISSUER`, `JWT_AUDIENCE`) — see
...
**real** shared `datacern_common` adapters against local, protocol-compatible
...
|---|---|---|
| `SecretsStore` (`domain/secrets.py`) | `InMemorySecretsStore` | `VaultSecretsStore` → `datacern_common.secrets` (**Vault KV v2** via hvac; 7-day grace destroy sweeper) |
| `ObjectStore` (`domain/objectstore.py`) | `LocalFSObjectStore` (streaming, temp+rename) | `S3ObjectStore` → `datacern_common.objectstore` (**MinIO/S3** multipart streaming, ≤1 part buffered, sha256 etag) |
| `TableWriter` (`domain/tablewriter.py`) | `ParquetFileTableWriter` (parquet + snapshot ledger) | `IcebergTableWriter` → `datacern_common.iceberg` (**Iceberg REST catalog + MinIO** via pyiceberg; two-phase stage/commit, BR-9 snapshot-summary `ingestion_id`) |
| `PolicyEngine` (`domain/policy.py`) | `StaticPolicyEngine` (deny-list double) | `OPAPolicyEngine` → `datacern_common.opaclient` (**OPA** `datacern.authz_input` + **Redis** projection) |
| `EventPublisher` (`events/outbox.py`) | `InMemoryEventPublisher` + `publish_pending` poller | `KafkaEventPublisher` → `datacern_common.kafka` (**Redpanda/Kafka** idempotent producer, tenant-keyed) |
| JWT keys (`api/auth.py`) | static RS256 public key | `JWKSKeyProvider` → `datacern_common.authjwt` (**cached JWKS refresh**, RS256, alg=none rejected) |
