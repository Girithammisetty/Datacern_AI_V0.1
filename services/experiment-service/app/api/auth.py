"""AuthN/AuthZ (MASTER-FR-010..016).

- External requests: RS256 JWT verified against a static PEM (dev/tests) or
  cached JWKS (prod). ``alg=none``/non-RS256 rejected by construction.
- Internal requests: SPIFFE identity header (mesh mTLS) + HMAC body signature
  for MLflow webhook deliveries (EXP-FR-010).
- Authorization: local scope check (unit) behind the AuthzClient port; the real
  OPA sidecar adapter (MASTER-FR-012) is the runtime default.
"""
...
class Principal:
    ...
    def ctx(self, trace_id: str | None = None, workspace_id: str | None = None) -> CallCtx:
        return CallCtx(tenant_id=self.tenant_id, actor=self.actor, via_agent=self.via_agent,
                       trace_id=trace_id, workspace_id=workspace_id or self.workspace_id)
...
class TokenVerifier:
    ...
    async def verify(self, token: str) -> Principal:
        ...
        if isinstance(scopes, str):
            scopes = scopes.split()
        return Principal(
            sub=claims["sub"], tenant_id=claims["tenant_id"], typ=claims.get("typ", "user"),
            scopes=list(scopes), agent_id=claims.get("agent_id"),
            agent_version=claims.get("agent_version"), obo_sub=claims.get("obo_sub"),
...
class OpaAuthzClient:
    ...
    async def allow(self, principal: Principal, action: str, resource_urn: str | None) -> bool:
        ...
        return await self._client.allow(
            ...
            resource_urn=resource_urn, workspace_id=principal.workspace_id, projection=proj)
