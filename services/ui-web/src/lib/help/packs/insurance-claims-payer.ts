...
export const insuranceClaimsPayerGuide: PackGuide = {
  ...
  "summary": "AI-assisted **prior-authorization review, appeal analysis, and denial analytics** for US health payers. New PA requests and provider appeals land as cases with the **CMS decision clock** already running (72 hours for urgent, 7 days for standard), a domain-grounded assistant drafts a recommended disposition, and a qualified human reviewer approves every determination — because under the CMS 2024 MA rule an adverse decision can never be made by the AI alone.\n\nIt ships the payer KPI semantic models, three operational dashboards, PA/appeal review queues with a full disposition taxonomy, the payer role catalog, and a PA-approval training pipeline — grounded throughout in plan policy, clinical guidelines, and denial reason codes (CARC).",
  "ships": [
    {
    ...
    },
    {
      "label": "AI & grounding",
      "items": [
        "A PA-nurse triage assistant and a payer-operations analytics agent, specialized to the domain",
  ...
  "personas": [
    ...
    },
    {
      ...
      "steps": "You hold clinical authority on the desk. You're the one who holds **approve**, so a determination becomes real only when you sign it — the human reviewer the CMS 2024 MA rule requires for every adverse decision.\n\n1. **Clear the approvals inbox.** Sidebar → **Approvals**. Each item shows the proposed disposition, the reviewer who proposed it, and the assistant's reasoning and cited criteria. **Approve** to write it back or **Reject** with a note. You **can't** approve a proposal you authored. *(See \"Approvals & four-eyes\".)*\n2. **Take the escalations.** Filter **Cases** to *Escalate to medical director* — the below-threshold PAs and specialty-drug denials the nurses sent up. *(See \"Your worklist\".)*\n3. **Decide on the cockpit.** Review the clinical picture and evidence, use the assistant to summarize plan policy and precedent, then record *PA approved*, *PA denied (human decision)*, or send an appeal to *Appeal upheld* / *Appeal overturned*. *(See \"Working a case\" and \"The assistant\".)*\n4. **Watch turnaround.** The **PA Ops** dashboard shows queue by urgency and average decision hours against the CMS windows — reassign from the **worklist** before anything breaches. *(See \"Dashboards\".)*\n5. **Auto-escalate the ones that can't wait for triage.** In **Cases → Settings → Triggers**, set a condition on the PA/appeal dataset (e.g. requested amount over your specialty-drug threshold, or urgency = urgent) so those requests open as escalated cases the instant they're ingested, instead of depending on a nurse to flag them. *(See \"Automating case creation\".)*\n6. **Clear backlogs.** When a batch of routine standard PAs is clean, bulk-approve to keep the queue moving without letting urgent cases wait."
    },
    {
