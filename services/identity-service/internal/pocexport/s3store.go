...
// S3Store is the real MinIO/S3-compatible object store (prod default, safe
...
func (s *S3Store) Put(ctx context.Context, key string, data []byte, ttl time.Duration) (string, time.Time, error) {
	if _, err := s.Client.Put(ctx, key, data, "application/json"); err != nil {
