...
class Principal:
    ...
    def effective_user(self) -> str:
        return self.obo_sub if (self.typ == "agent_obo" and self.obo_sub) else self.sub

    def ctx(self, trace_id: str | None = None) -> CallCtx:
        return CallCtx(
            tenant_id=self.tenant_id, actor=self.actor, via_agent=self.via_agent,
            trace_id=trace_id, subject=self.sub, typ=self.typ, obo_sub=self.obo_sub,
            scopes=list(self.scopes), agent_id=self.agent_id,
...
class LocalScopeAuthz:
    """Scope-based allow decision (unit/dev double). NOT in the real container."""
...
class OpaAuthzClient:
    ...
    async def allow(self, principal: Principal, action: str, resource_urn: str | None) -> bool:
        ...
        # memory.* actions) load the per-workspace projection key and satisfy
