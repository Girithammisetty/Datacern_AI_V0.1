...
export const typeDefs = gql`
  ...
  """One audit event with its hash-chain position (audit-service GET
  /audit/events/{event_id}). \`sealed\` is true once the event's chain-day has
  been sealed by the daily WORM export — the record is then verifiable against
  its Object-Lock manifest."""
  type AuditEventDetail {
    event: AuditEvent!
    """YYYY-MM-DD chain-day the event was sequenced into."""
    chainDate: String!
    chainSeq: Int!
    sealed: Boolean!
  }

  """One sealed WORM export batch (audit-service GET /exports, AUD-FR-023): a
  chain-day's tamper-evident manifest in Object-Lock storage. \`downloadUrl\`
  is a presigned manifest link (absent when signing was unavailable)."""
  type AuditExportBatch {
    """YYYY-MM-DD chain-day the batch covers."""
    date: String!
    """Manifest revision for the day (re-seals after supplements increment it)."""
    revision: Int!
    """The immutable s3:// object URI of the manifest."""
    uri: String!
  ...
  enum AuditExportFormat { CSV NDJSON }

  """Descriptor for a raw audit-trail file export (audit-service GET
  /audit/export, capability audit.event.export — a streaming CSV/NDJSON
  download of up to 100k rows). The file body never traverses GraphQL:
  \`downloadPath\` is the same-origin ui-web proxy route (/api/audit-export)
  that streams it from audit-service with the caller's session JWT. \`from\`/
  \`to\` echo the validated window the link will export."""
  ...
  """Effective execution ceilings (query-service domain.Ceilings). Byte/row
  counts are Float — they exceed GraphQL's 32-bit Int (50 GB scan cap)."""
  type QueryCeilings {
    maxScanBytes: Float
    maxRuntimeS: Float
    maxResultBytes: Float
    maxResultRows: Float
  }

  """The tenant's stored override row (query-service domain.TenantLimits) —
  every field nullable; null means the platform default applies."""
  type QueryLimitOverrides {
    maxScanBytes: Float
    maxRuntimeS: Float
    maxResultBytes: Float
    maxResultRows: Float
  ...
  """
  Tenant query governance (query-service GET /limits, QRY-FR-042/044): the
  stored overrides plus the recomputed effective ceilings for human and agent
  callers, and the platform maxima overrides may never exceed.
  """
  type QueryLimits {
  ...
  """
  PUT /limits body (query-service). Overrides may only LOWER ceilings — a value
  above the platform maximum 422s with the allowed range in details. Omitted
  fields keep their current override; concurrentSlots caps simultaneous
  executions per tenant (QRY-FR-044).
  """
  input QueryLimitsInput {
    maxScanBytes: Float
    maxRuntimeS: Float
    maxResultBytes: Float
    maxResultRows: Float
  ...
  QRY-FR-041). ceilingVerdict is ok | exceeded; partitionsPruned is a
  "pruned/total" summary string; confidence is high | low.
  """
  type SqlDryRun {
    engine: String
    routingReason: String
    estimatedScanBytes: Float
    estimatedRows: Float
  ...
  QRY-FR-062). downloadPath is the SERVICE-RELATIVE pre-authenticated path
  /api/v1/downloads/{token} — the UI streams it through its same-origin
  /api/query-export/{token} proxy (an HMAC-signed link, valid 24h; the download
  itself needs no JWT). Expired results answer 410 with a re-run hint.
  """
  type QueryExportDescriptor {
    """csv (parquet is a documented 501 stub downstream)."""
    format: String!
    downloadPath: String!
    expiresAt: DateTime
  }
  ...
  """
  A page of RAW rows behind a clicked aggregate segment (chart-service POST
  /charts/{id}/drilldown, CHART-FR-040). columns/rows mirror query-service's
  execution-result shape; nextCursor pages forward.
  """
  type ChartDrilldownRows {
    columns: JSON
    rows: JSON
    nextCursor: String
    hasMore: Boolean!
  ...
  """
  Drilldown request (chart-service resolve.DrilldownRequest): the clicked
  dimension/value is injected as a bind predicate over the chart's configured
  drilldown saved query (display_meta.drilldown.query_urn — 422 NO_DRILLDOWN
  when absent). filters use the same op whitelist as ChartFilterInput; limit
  caps at 200 (default 50).
  """
  input ChartDrilldownInput {
    dimension: String!
    value: JSON
    """Series discriminator value when the chart splits by a dataseries column."""
  ...
  """
  An async chart-export operation (chart-service domain.Operation,
  CHART-FR-041). status: pending | running | completed | failed. artifactUrl is
  set once completed — FSStore mints a chart-service-relative signed path
  (/api/v1/exports/…?exp&sig); S3Store an absolute presigned URL. The UI
  downloads through its same-origin /api/chart-export/{operationId} proxy.
  """
  type ChartExportOperation {
    id: ID!
    chartId: ID
    kind: String
    """csv | png (png is infra-gated on the headless renderer sidecar)."""
    format: String
    status: String!
    artifactUrl: String
    artifactUrn: String
  ...
  """Result of importing a dashboard bundle (chart-service POST /dashboards/import)."""
  type DashboardImportResult {
    dashboardId: ID!
    chartsCreated: Int!
  }

  """A parent→child chart cross-navigation link (chart-service PUT /charts/{id}/link,
  CHART-FR-015). linkType: 0 = shared-source, 1 = main-secondary."""
  type ChartLink {
    parentChartId: ID!
    childChartId: ID!
    linkType: Int!
  ...
  input ChartLinkColumnInput {
    parentCol: String!
    childCol: String!
  }

  input ChartLinkInput {
    childChartId: ID!
    linkedColumns: [ChartLinkColumnInput!]
    """0 = shared-source, 1 = main-secondary (default 1)."""
    linkType: Int
  }
  ...
  "Grouping for the report catalog — what a report is about."
  enum ReportDomain { financial governance operations }
  "A downloadable/exportable format a report offers."
  enum ReportFormat { csv json artifact }
  "How often a report is meaningfully produced."
  enum ReportCadence { on_demand monthly point_in_time }

  """
  One entry in the report catalog (Insights › Reports). Server-authoritative:
  the BFF owns the catalog and returns only the reports the caller's
  capabilities can reach, so the UI renders a filtered list rather than deciding
  visibility client-side. Each entry deep-links (href) to where the report runs.
  """
  type ReportDefinition {
    ...
    schedulable: Boolean!
    "A real constraint disclosed on the card (e.g. finalized months only)."
    note: String
  }
  ...
  """Aging bucket over OPEN cases (closed cases have no age to worry about)."""
  type QueueAgingBucket {
    label: String!
    count: Int!
  }
  type QueueOpenCounts {
    total: Int!
    unassigned: Int!
    inProgress: Int!
  }
  type QueueSla {
    """Past due and still open — already a miss."""
    breached: Int!
    """Open, not yet breached, due inside a day: the set still savable."""
    dueWithin24h: Int!
  }
  ...
  p50/p90 are NULLABLE ON PURPOSE: null means nothing resolved in the window.
  They must never be coerced to 0, which would read as instant decisions.
  'sample' is the count they were computed from, so a p90 over three cases is
  visibly not a trend."""
  type QueueLatency {
    p50Seconds: Int
    p90Seconds: Int
  ...
  """The approver's view of the queue: how old the backlog is, what breaches
  next, how much is clearing, and how long a decision takes. Composed from data
  case-service already stores; scoped to the caller's own workspace."""
  type QueueIntelligence {
    generatedAt: String!
    windowDays: Int!
    open: QueueOpenCounts!
    aging: [QueueAgingBucket!]!
    sla: QueueSla!
  ...
  """Result of a governed memory write (memory-service WriteResult). \`status\`:
  active | quarantined | queued (embedding degraded, write pended) — batch rows
  may also be rejected, carrying \`code\`/\`message\` instead of failing the
  whole batch. \`memoryId\` is null for session-scope and queued writes."""
  type MemoryWriteResult {
    memoryId: ID
    status: String!
    """True when the write deduplicated into an existing similar record."""
    merged: Boolean
    """True for session-scope writes (Redis only, no durable record)."""
    session: Boolean
    """Rejection error code (batch rows only)."""
    code: String
    """Rejection error message (batch rows only)."""
    message: String
  }

  """Provenance of a memory write (memory-service ProvenanceIn). \`sourceType\`:
  agent_run | user_explicit | tool_output | admin."""
  ...
  input WriteMemoryInput {
    """session | user | workspace | tenant. Users may write only their own
    user/session scope; tenant scope needs memory.corpus.admin."""
    scope: String!
    scopeRef: String!
    content: String!
  ...
  input MemoryRetrievalTestInput {
    queryText: String
    """Scopes to search: user | workspace | tenant (session is a separate
    downstream path and is ignored here)."""
    scopes: [String!]!
    """scope -> scope_ref map. Only \`workspace\` needs one (and the caller must
    be a member); user/tenant refs are server-resolved from the principal."""
    scopeRefs: JSON
    """RAG corpus keys to search alongside memory scopes."""
    corpora: [String!]
    topK: Int
    minConfidence: Float
    tags: [String!]
  ...
  """One ranked retrieval hit (memory-service POST /retrieve). \`kind\` is
  memory (scope/memoryId/provenance populated) or chunk (corpus/chunkId/
  sourceUrn/snapshotVer populated). \`content\` is UNTRUSTED model input
  (BR-12) — the downstream marks every hit contentDisposition "untrusted"."""
  type MemoryRetrievalHit {
    ...
    sourceUrn: String
    snapshotVer: String
    """Ranking internals, only when includeDebug was set."""
    debug: JSON
  }

  """Retrieval tester result. \`degraded\` is the downstream envelope flag —
  true when ANN search ran in a degraded mode."""
  type MemoryRetrievalResult {
    hits: [MemoryRetrievalHit!]!
    degraded: Boolean!
  }

  """A tenant RAG corpus (memory-service _corpus_view). \`status\`: active |
  paused | rebuilding. \`source\`/\`chunking\`/\`refresh\`/\`anonymizationProfile\`
  are downstream config dicts passed through as JSON."""
  type RagCorpus {
    corpusKey: ID!
    source: JSON
    chunking: JSON
    ...
    refresh: JSON
    anonymizationProfile: JSON
    status: String!
  }

  """Corpus health (memory-service GET /corpora/{key}/status)."""
  type CorpusStatus {
    corpusKey: ID!
    status: String!
    activeEmbeddingVer: String!
  ...
  """Report from a corpus re-embed (memory-service POST /corpora/{key}/rebuild)."""
  type CorpusRebuildReport {
    corpusKey: ID!
    activeEmbeddingVer: String!
    chunksReembedded: Int!
    oldChunksDropped: Int!
  }

  """Result of pushing a document into the docs corpus."""
  type CorpusDocumentPushResult {
    sourceUrn: String!
    chunks: Int!
  }

  input RegisterCorpusInput {
    corpusKey: String!
    source: JSON
    chunking: JSON
  ...
  input CorpusPatchInput {
    source: JSON
    chunking: JSON
    refresh: JSON
    anonymizationProfile: JSON
    """active | paused (rebuilding is set by the rebuild flow, not patchable in
    practice)."""
    status: String
  }

  """The tenant memory PII/retention policy (memory-service GET/PUT
  /policies/self). \`ttlOverrides\` maps scope -> ISO-8601 duration;
  \`corpusFlags\` maps corpus_key -> enabled."""
  type MemoryPolicy {
    ttlOverrides: JSON!
    piiClasses: [String!]!
  ...
  Platform-defined, versioned catalog — no tenant-defined meters in v1; the
  canonical \`unit\` never changes post-launch (USG-FR-005)."""
  type UsageMeter {
    meterKey: String!
    unit: String!
    ...
    aggregation: String!
    description: String!
    """Dimension names carried on this meter's records (documentation of the
    rollup contract, not a per-row guarantee)."""
    dimensions: [String!]!
    deprecated: Boolean!
  }

  """One priced monthly chargeback row (usage-service GET /reports/chargeback,
  USG-FR-043). \`usd\` is quantity x pricePerUnitUsd at the rate card resolved
  for month start (BR-5); \`totalUsd\` folds in \`adjustmentsUsd\` (signed
  adjustment deltas recorded on the closed month). rateCardId is null when no
  rate card priced the meter."""
  type ChargebackLine {
    tenantId: String!
    workspaceId: String
    """YYYY-MM."""
  ...
  reconciliation sits in variance (AC-9) until acknowledged/adjusted."""
  type Reconciliation implements Node {
    id: ID!
    urn: String!
    """YYYY-MM."""
    month: String!
  ...
  """A recorded billing adjustment/credit (usage-service POST /adjustments 201
  echo, USG-FR-072). Append-only, actor-signed downstream; the route echoes
  only these fields (no createdAt/actor) — nothing here is fabricated."""
  type UsageAdjustment implements Node {
    id: ID!
    urn: String!
    meterKey: String!
    """YYYY-MM (must already be finalized)."""
    month: String!
    quantityDelta: Float!
    usdDelta: Float!
    reason: String!
  ...
  /adjustments, USG-FR-072). \`month\` must be finalized — an open month 409s
  with reason month_open (BR-5). \`usdDelta\` is signed (negative = credit)."""
  input CreateUsageAdjustmentInput {
    meterKey: String!
    month: String!
    quantityDelta: Float!
    usdDelta: Float!
  ...
  type Query {
    ...
    /audit/agent-activity): every event the agent performed, each row carrying
    who it acted on behalf of (viaAgentId = the agent, actorId = the human
    principal on OBO rows). \`oboUserId\` narrows to a single on-behalf-of
    user; \`includeAutonomous\` adds rows with no human principal. \`from\`/
    \`to\` default to the backend's max 92-day window. Capped at 200 rows,
    newest first — the downstream route never paginates, so pageInfo.hasMore
    is always false. Needs audit.event.read."""
    auditAgentActivity(
      agentId: ID!
      oboUserId: String
      from: DateTime
      to: DateTime
      includeAutonomous: Boolean = false
    ): AuditEventConnection!

    """One audit event by id with its chain position (audit-service GET
    /audit/events/{event_id}). Null for cross-tenant and nonexistent alike —
    the downstream 404s both identically (MASTER-FR-003). Needs
    audit.event.read."""
    auditEvent(id: ID!): AuditEventDetail

    """Previously generated WORM export batches (audit-service GET /exports):
    the sealed per-day Object-Lock manifests, optionally narrowed to one
    YYYY-MM-DD day. Bounded list — the downstream route never paginates.
    Needs audit.export.read."""
    auditExports(date: String): [AuditExportBatch!]!

    """Descriptor for a raw audit-trail export download (audit-service GET
    /audit/export — streaming CSV/NDJSON, capability audit.event.export). The
    BFF validates the window against the downstream's real rules (from/to
    required there, max 92 days; defaults to the last 7 days) and returns the
    same-origin \`downloadPath\` that ui-web's zero-logic proxy
    (/api/audit-export) streams the file through — the body itself is never
    proxied through GraphQL. Filters mirror auditEvents."""
    auditExportFile(
      from: DateTime
      to: DateTime
      eventType: String
    ...
    """The tenant's query-governance ceilings + concurrency (query-service GET
    /limits, QRY-FR-042/044): stored overrides, effective user/agent ceilings,
    ...
    """
    The report catalog for Insights › Reports — every report the CALLER can
    reach, filtered server-side by their rbac capabilities. The UI renders this
    list directly instead of deciding visibility client-side. Empty (never an
    error) when capabilities can't be resolved, so the hub degrades to just its
    ...
    """
    Raw rows behind a clicked aggregate segment (chart-service POST
    /charts/{id}/drilldown, CHART-FR-040). A Query, not a Mutation: the
    downstream handler is side-effect free — it wraps the chart's drilldown
    saved query with a bind predicate and reads. 422 NO_DRILLDOWN when the
    chart declares none. Needs chart.chart.read.
    """
    chartDrilldown(chartId: ID!, input: ChartDrilldownInput!): ChartDrilldownRows!
    """An async chart-export operation's status + artifact link (chart-service
    GET /operations/{id}) — poll after exportChart until completed|failed."""
    chartExportOperation(id: ID!): ChartExportOperation
    ...
    """Approver queue intelligence (case-service GET /cases/queue-intelligence):
    aging, SLA risk, throughput and time-to-decision for the caller's OWN
    workspace. There is deliberately no workspace argument — the downstream
    derives it from the caller's claims, and accepting one here would let a
    client count another department's backlog. Needs case.case.read."""
    queueIntelligence(days: Int = 7): QueueIntelligence!
    ...
    """The seeded platform meter catalog (usage-service GET /meters,
    USG-FR-003). Needs usage.meter.read."""
    usageMeters: [UsageMeter!]!
    """Priced monthly chargeback rollups for a finalized month (usage-service
    GET /reports/chargeback, USG-FR-043). \`month\` is YYYY-MM. 409s verbatim
    ...
    platform-operator token; a tenant admin gets a real 403, never faked."""
    ...
    memoryStats: JSON!
    """
    Retrieval tester — "what would the agent recall for this query" (memory-service
    POST /retrieve; read-only despite the POST). Scope refs are server-verified:
    user/tenant resolve from the caller, workspace requires membership. Needs
    memory.memory.read.
    """
    memoryRetrievalTest(input: MemoryRetrievalTestInput!): MemoryRetrievalResult!
    """Corpus health: status + active embedding version + chunk count
    (memory-service GET /corpora/{key}/status). Needs memory.corpus.admin."""
    corpusStatus(corpusKey: String!): CorpusStatus
    """The caller's tenant memory PII/retention policy (memory-service GET
    /policies/self). Needs memory.policy.read."""
  ...
  type Mutation {
    ...
    /reconciliations/{id}/acknowledge, USG-FR-070/071) — unblocks chargeback
    for that month. Only a row in \`variance\` can be acknowledged (anything
    else 404s downstream, surfaced verbatim). Platform-only — needs
    usage.reconciliation.update AND a platform-operator token."""
    acknowledgeReconciliation(id: ID!): Reconciliation!
    """Record a signed billing adjustment/credit on a CLOSED month
    (usage-service POST /adjustments, USG-FR-072). An open month 409s with
    reason month_open (BR-5), surfaced verbatim. Platform-only — needs
    usage.reconciliation.update AND a platform-operator token."""
    createUsageAdjustment(input: CreateUsageAdjustmentInput!): UsageAdjustment!
    ...
    """
    Set the tenant's query-governance overrides (query-service PUT /limits,
    QRY-FR-042/044). Values may only LOWER the platform maxima (422 above
    them). The PUT answers only the stored overrides, so this re-reads GET
    /limits to return the recomputed effective view. Needs query.limits.update.
    """
    setQueryLimits(input: QueryLimitsInput!): QueryLimits!
    """
    Plan + estimate ad-hoc SQL WITHOUT executing (query-service POST
    /sql/dry-run, QRY-FR-041): engine routing, estimated scan bytes/rows,
    ceiling verdict. A ceiling-exceeding statement can also 422
    COST_CEILING_EXCEEDED with the estimate in error details. Needs
    query.execution.execute.
    """
    dryRunSql(input: RunSqlInput!): SqlDryRun!
    """
    Mint a signed CSV download link for a SUCCEEDED execution's results
    (query-service POST /executions/{id}/export, QRY-FR-062). The descriptor's
    downloadPath is pre-authenticated by HMAC signature (24h) — stream it
    through the UI's same-origin /api/query-export/{token} proxy, not GraphQL.
    409 while the execution is not succeeded; 410 once results expired. Needs
    query.execution.export.
    """
    exportQueryExecution(id: ID!, format: String): QueryExportDescriptor!
    ...
    """
    Start an async CSV/PNG chart export (chart-service POST /charts/{id}/export,
    CHART-FR-041; 202, capped at 5 concurrent exports/tenant → 429
    EXPORT_LIMIT). The 202 carries only {operation_id}; this re-reads the
    operation so the caller gets its REAL current state — poll
    chartExportOperation until completed|failed, then download the artifact via
    the UI's /api/chart-export/{operationId} proxy. PNG is infra-gated on the
    renderer sidecar (documented failure). Needs chart.chart.export.
    """
    exportChart(id: ID!, format: String!, request: JSON): ChartExportOperation!
    """
    Export a dashboard + its charts as a self-contained portable JSON bundle
    (chart-service POST /dashboards/{id}/export-bundle, CHART-FR-005). Plain
    JSON — the UI offers it as a client-side file download, no proxy needed.
    Needs chart.dashboard.export.
    """
    exportDashboardBundle(id: ID!): JSON!
    """
    Import a dashboard bundle into the caller's workspace (chart-service POST
    /dashboards/import, CHART-FR-005/BR-10). urnMapping remaps the bundle's
    source URNs to this tenant's; any unmapped URN fails atomically with 422
    UNMAPPED_URN before anything is written. The target workspace comes from
    the caller's JWT \`workspace_id\` claim (as createDashboard). Needs
    chart.dashboard.create.
    """
    importDashboard(bundle: JSON!, urnMapping: JSON, idempotencyKey: String): DashboardImportResult!
    """
    Create/replace a parent→child chart cross-navigation link (chart-service
    PUT /charts/{id}/link, CHART-FR-015) — transactional with cycle detection
    (409 on a cycle). Authorized downstream as chart.chart.update.
    """
    setChartLink(chartId: ID!, input: ChartLinkInput!): ChartLink!
    """Remove a chart link + clear the child back-reference (chart-service
    DELETE /charts/{id}/link, 204). Needs chart.chart.update."""
    deleteChartLink(chartId: ID!, childChartId: ID!): Boolean!
    ...
    """
    Governed memory write (memory-service POST /memories). Scope rules are
    enforced downstream: users may write only their own user/session scope;
    tenant-scope grounding additionally needs memory.corpus.admin. Needs
    memory.memory.create.
    """
    writeMemory(input: WriteMemoryInput!): MemoryWriteResult!
    """Batch memory write (memory-service POST /memories/batch) — per-item
    results; rejected items carry code/message instead of failing the batch.
    Needs memory.memory.create."""
    writeMemoryBatch(items: [WriteMemoryInput!]!): [MemoryWriteResult!]!
    """Edit a memory record's content (memory-service PATCH /memories/{id});
    returns the full updated record. Needs memory.memory.update."""
    updateMemory(id: ID!, content: String!): MemoryRecord!
    """Delete a memory record (memory-service DELETE /memories/{id}, 204).
    Needs memory.memory.delete."""
    deleteMemory(id: ID!): Boolean!
    """
    Release a quarantined memory record back to active (memory-service POST
    /memories/{id}/unquarantine). \`reason\` is REQUIRED downstream and audited.
    Needs memory.memory.update.
    """
    unquarantineMemory(id: ID!, reason: String!): MemoryRecord!
    """Register a tenant RAG corpus (memory-service POST /corpora, 201).
    Needs memory.corpus.admin."""
    registerCorpus(input: RegisterCorpusInput!): RagCorpus!
    """Patch a corpus' source/chunking/refresh/anonymization/status
    (memory-service PATCH /corpora/{key}). Needs memory.corpus.admin."""
    updateCorpus(corpusKey: String!, patch: CorpusPatchInput!): RagCorpus!
    """
    Re-embed a corpus under a NEW embedding model version (memory-service POST
    /corpora/{key}/rebuild, 202) — runs synchronously downstream and 409s when
    the version is unchanged or a rebuild is already in progress. Needs
    memory.corpus.admin.
    """
    rebuildCorpus(corpusKey: String!, embeddingModelVer: String!): CorpusRebuildReport!
    """Push a document into the "docs" corpus (memory-service POST
    /corpora/docs/documents, 201) — replaces any prior chunks for the same
    sourceUrn. Needs memory.corpus.admin."""
    pushCorpusDocument(sourceUrn: String!, content: String!): CorpusDocumentPushResult!
    """
    Replace the tenant memory PII/retention policy (memory-service PUT
    /policies/self) — a full-document PUT, not a merge: omitted fields reset to
    downstream defaults. Needs memory.policy.update.
    """
    setMemoryPolicy(input: MemoryPolicyInput!): MemoryPolicy!
