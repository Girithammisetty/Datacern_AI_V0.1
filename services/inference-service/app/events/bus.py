"""Event bus + consumer dedup implementations (in-memory doubles + real Kafka)."""
...
class InMemoryEventBus:
    """In-memory fake of the Kafka bus (unit/dev). Records envelopes and
    dispatches to subscribers. Test double only."""
...
class KafkaEventBus:
    """Real Kafka (Redpanda) event bus via the shared idempotent producer;
    publishes the master envelope keyed by tenant_id. Runtime event bus."""
...
class InMemoryDedupStore:
    """Dedup fake (MASTER-FR-032; Redis in prod). Test double only."""
...
class RedisDedupStore:
    """Real Redis consumer dedup (24h TTL, MASTER-FR-032). Runtime dedup store."""
    ...
    async def claim(self, tenant_id: str, event_id: str) -> bool:
        return await self._store.claim(tenant_id, event_id)
