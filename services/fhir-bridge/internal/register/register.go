...
// (RBC-FR-022): fhir-bridge pushes its action manifest to rbac's idempotent
...
// Register mints a service token and POSTs fhir-bridge's action manifest to
...
// (/readyz reports 503 until registration succeeds — the register.Status
// pattern).
func Register(ctx context.Context, cfg Config) error {
	...
	// Retry a few times: rbac may still be coming up when fhir-bridge boots.
	var lastErr error
...
func mintServiceToken(key *rsa.PrivateKey, cfg Config) (string, error) {
	...
	claims := jwt.MapClaims{
		...
		"jti":       fmt.Sprintf("fhir-bridge-register-%d", now.UnixNano()),
	}
