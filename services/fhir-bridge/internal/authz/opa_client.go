...
func NewOPAClient(opaURL, redisAddr string) *OPAClient {
	...
	// Redis-miss fallback (RBC-FR-045): a Redis restart/failover must not deny
	// the whole tenant — self-heal from rbac-service's SQL ground truth,
	// reusing the deploy-time registration credential (RBAC_URL/REGISTER_SIGNING_*).
