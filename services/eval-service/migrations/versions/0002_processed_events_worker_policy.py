...
0001's `worker_outbox` policy on `outbox` (same rationale as dataset-service's
0005 / memory-service's 0003 identical migrations).
...
every row -- not an error, just a silent no-op, exactly like 0001's `outbox`
before `worker_outbox` was added. Same fix, same table shape.
