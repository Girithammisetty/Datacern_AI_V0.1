...
async def test_ac10_cloud_affinity_beats_priority(client, container):
    ...
    assert r.headers["x-datacern-deployment"] == gcp.id
    span = container.tracer.spans_named("chat")[-1]
    assert span.attributes.get("datacern.routing.cross_cloud") in (None, False)
...
async def test_cross_cloud_only_when_no_same_cloud(client, container):
    ...
    r = await client.post("/v1/chat/completions", json=CHAT_BODY,
                          headers=dp_headers(secret, token=token))
    assert r.status_code == 200
    assert r.headers["x-datacern-deployment"] == aws.id
    span = container.tracer.spans_named("chat")[-1]
    assert span.attributes["datacern.routing.cross_cloud"] is True
...
async def test_ac7_failover_after_two_500s(client, container):
    ...
    assert r.headers["x-datacern-deployment"] == d2.id
    span = container.tracer.spans_named("chat")[-1]
    attempts = span.attributes["datacern.routing.attempts"]
...
async def test_ac7_all_fail_returns_503(client, container):
    ...
    span = container.tracer.spans_named("chat")[-1]
    assert span.attributes["datacern.rejected_stage"] == "provider"
...
async def test_br8_draining_falls_through_to_next_rung_up(client, container):
    ...
    r = await client.post("/v1/chat/completions", json=CHAT_BODY,
                          headers=dp_headers(secret))
    assert r.status_code == 200
    assert r.headers["x-datacern-deployment"] == balanced.id
    span = container.tracer.spans_named("chat")[-1]
    assert span.attributes["datacern.routing.rung_fallback"] == "up"
...
async def test_escalation_serves_next_rung(client, container):
    ...
    r1 = await client.post("/v1/chat/completions", json=CHAT_BODY,
                           headers=dp_headers(secret))
    assert r1.headers["x-datacern-rung"] == "0"
    r2 = await client.post(
        "/v1/chat/completions", json=CHAT_BODY,
        headers=dp_headers(secret, **{
            "x-datacern-escalate": "true",
            "x-datacern-prior-request-id": r1.headers["x-datacern-request-id"],
        }),
    )
    assert r2.status_code == 200
    assert r2.headers["x-datacern-rung"] == "1"
    span = container.tracer.spans_named("chat")[-1]
    assert span.attributes["datacern.escalated"] is True
...
async def test_min_rung_header(client, container):
    ...
    _, secret = await mint_key(container)
    r = await client.post("/v1/chat/completions", json=CHAT_BODY,
                          headers=dp_headers(secret, **{"x-datacern-min-rung": "1"}))
    assert r.status_code == 200
