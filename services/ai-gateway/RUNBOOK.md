...
| Requests silently at rung 0 with `x-datacern-degraded: budget` | Budget window ≥ degrade_pct (default 95%) | Expected FinOps behavior; raise budget or wait for window reset. |
| Cache hit rate drops to 0 for a tenant | Guardrail policy or ladder change rotated context_hash (by design) | No action; cache re-warms. Verify with `datacern.cache` span attr. |
