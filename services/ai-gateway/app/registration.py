...
ai-gateway pushes its action manifest to rbac's idempotent registration API at
...
async def register_actions(settings) -> bool:
    if not settings.rbac_url or not settings.register_signing_key_pem:
        logger.info("ai-gateway action registration skipped (rbac_url/signing key unset)")
        return False
    try:
        token = _mint_service_token(settings)
    except Exception as exc:  # noqa: BLE001
        logger.warning("ai-gateway action registration: token mint failed: %s", exc)
        return False
        ...
        for attempt in range(10):
            try:
                ...
                if resp.status_code == 200:
                    logger.info("ai-gateway action catalog registered (%d actions)", len(MANIFEST))
                    return True
                last = f"{resp.status_code} {resp.text[:200]}"
            except Exception as exc:  # noqa: BLE001
                last = str(exc)
            await asyncio.sleep(0.5 * (attempt + 1))
    logger.warning("ai-gateway action registration failed: %s", last)
    return False
