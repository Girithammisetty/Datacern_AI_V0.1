...
def _build_ctx(request: Request, default_class: str = "chat") -> RequestCtx:
    ...
    request_class = request.headers.get("x-datacern-request-class", default_class)
    if request_class not in REQUEST_CLASSES:
        raise ValidationFailed(
            f"unknown request class {request_class!r}",
            details=[{"field": "x-datacern-request-class",
                      "problem": f"allowed: {list(REQUEST_CLASSES)}"}],
        )
    # AIG-FR-002: x-datacern-tenant-id is ignored; tenant comes from the JWT.
    # Attribution headers are validated against JWT claims where overlapping.
    agent_id = request.headers.get("x-datacern-agent-id")
    agent_version = request.headers.get("x-datacern-agent-version")
    if principal.agent_id and agent_id and agent_id != principal.agent_id:
        raise ValidationFailed(
            "x-datacern-agent-id does not match the JWT agent_id claim"
        )
    if (principal.agent_version and agent_version
            and agent_version != principal.agent_version):
        raise ValidationFailed(
            "x-datacern-agent-version does not match the JWT claim"
        )
    min_rung_raw = request.headers.get("x-datacern-min-rung")
    try:
        min_rung = int(min_rung_raw) if min_rung_raw is not None else None
    except ValueError as exc:
        raise ValidationFailed("x-datacern-min-rung must be an integer") from exc
    return RequestCtx(
        ...
        attribution=Attribution(
            workspace_id=request.headers.get("x-datacern-workspace-id"),
            ...
            tool=request.headers.get("x-datacern-tool"),
            feature=request.headers.get("x-datacern-feature"),
        ),
        cell_cloud=principal.cell_cloud,
        trace_id=getattr(request.state, "trace_id", None),
        escalate=_bool_header(request, "x-datacern-escalate"),
        prior_request_id=request.headers.get("x-datacern-prior-request-id"),
        min_rung=min_rung,
...
def _headers(ctx: RequestCtx, result: PipelineResult) -> dict:
    headers = {
        "x-datacern-request-id": ctx.request_id,
        "x-datacern-rung": str(result.rung),
        "x-datacern-cache": (
            "hit" if result.cache.startswith("hit") else result.cache
        ),
    }
    if result.deployment_id:
        headers["x-datacern-deployment"] = result.deployment_id  # AIG-FR-009b
    if result.degraded:
        headers["x-datacern-degraded"] = "budget"
    if result.guardrail_flags:
        headers["x-datacern-guardrail-flags"] = ",".join(result.guardrail_flags)
    return headers
