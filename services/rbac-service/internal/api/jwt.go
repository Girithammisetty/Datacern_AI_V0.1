...
// EffectiveUser is whose permissions apply (OBO -> original user).
...
// HasScope checks token scopes ("*" is the unrestricted wildcard).
...
// SuperAdmin: platform-operator override capability (last-admin override,
// admin projection APIs). Carried as a scope on service/user tokens.
const ScopeSuperAdmin = "super_admin"

// Verifier validates RS256 JWTs (MASTER-FR-010): signature via JWKS (cached,
// refreshed ≤ 5 min) or a static test key; exp/iss/aud validated. alg=none
// and non-RS256 algorithms are rejected outright (MASTER-FR-014).
