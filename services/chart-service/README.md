...
| BRD 74 D3 dashboard search (`GET /dashboards?q=`) | `handlers_dashboards.go` `handleListDashboards`, `store/pg.go` `ListDashboards` + `domain.DashboardListFilter` | `internal/api/dashboardsearch_test.go` (8), `test/integration/dashboardsearch_test.go` (real SQL + RLS) |
