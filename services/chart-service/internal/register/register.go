// Package register performs deploy-time action-catalog registration
// (RBC-FR-022): chart-service pushes its action manifest to rbac's idempotent
// registration API at startup so OPA's catalog knows every action this service
// authorizes against. In production the call carries the service's SPIFFE mTLS
// identity; in dev/e2e it mints a short-lived service JWT signed with the
...
// Config carries the endpoint + credentials for registration.
type Config struct {
	RBACURL       string
...
// Register mints a service token and POSTs chart-service's action manifest to
...
func mintServiceToken(key *rsa.PrivateKey, cfg Config) (string, error) {
	...
	claims := jwt.MapClaims{
		"sub": "svc:chart-service", "typ": "service", "tenant_id": cfg.TenantID,
		"scopes": []string{"rbac.action.register"},
		"iss":    cfg.Issuer, "aud": cfg.Audience,
		"iat": now.Unix(), "exp": now.Add(5 * time.Minute).Unix(),
		"jti": fmt.Sprintf("chart-register-%d", now.UnixNano()),
	}
