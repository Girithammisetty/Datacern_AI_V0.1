...
// (RBC-FR-022): usage-service pushes its action manifest to rbac's idempotent
// registration API at startup so the catalog OPA consumes knows every action
// this service authorizes against. In dev/e2e the service mints a short-lived
...
// Register mints a service token and POSTs usage-service's action manifest to
...
func mintServiceToken(key *rsa.PrivateKey, cfg Config) (string, error) {
	...
	claims := jwt.MapClaims{
		...
		"jti":       fmt.Sprintf("usage-register-%d", now.UnixNano()),
	}
