...
pack-service registers its action manifest with rbac at startup so the OPA
catalog recognises (`action_known`) every action it authorizes against.
"""
...
# Actions pack-service authorizes against (all workspace-scoped).
MANIFEST: list[str] = [
    "pack.pack.read",          # browse the pack catalog
    "pack.install.read",       # read installs + their ledger
    "pack.install.execute",    # install / uninstall a pack into a workspace
]
...
async def register_actions(settings) -> bool:
    if not settings.rbac_url or not settings.register_signing_key_pem:
        logger.info("pack action registration skipped (rbac_url/signing key unset)")
        return False
    try:
        token = _mint_service_token(settings)
    except Exception as exc:  # noqa: BLE001
        logger.warning("pack action registration: token mint failed: %s", exc)
        return False
        ...
        for attempt in range(10):
            try:
                ...
                if resp.status_code == 200:
                    logger.info("pack action catalog registered (%d actions)", len(MANIFEST))
                    return True
                last = f"{resp.status_code} {resp.text[:200]}"
            except Exception as exc:  # noqa: BLE001
                last = str(exc)
            await asyncio.sleep(0.5 * (attempt + 1))
    logger.warning("pack action registration failed: %s", last)
    return False
