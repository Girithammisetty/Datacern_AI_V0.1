<!-- converted from Card_Disputes_AGENT_PLAYBOOK.docx by tools/docs/docx_to_md.py -->
...
DATACERN AI · CARD DISPUTES

Agent Playbook

How to put the nine Datacern agents to work in card disputes.

This playbook maps each of Datacern’s nine agents to a concrete job in this vertical, grounded in the real card-disputes pack.

Pack: card-disputes v1.0.0

Domain: banking · cards · disputes · chargebacks · fraud
...
AI-assisted card dispute and chargeback workflow for US issuers: dispute intake triage with regulatory-clock awareness (Reg E provisional credit, Reg Z billing-error windows), first-party fraud escalation, network chargeback recovery with reason-code grounding, dispute-operations KPI semantic model and dashboards (cardholder-favor rate, chargeback win rate, deadline runway, provisional-credit exposure), cardholder-merchant network analytics, Reg E/Z + network-rule grounding memories, and transaction-anomaly + dispute-outcome training pipelines.
...
| model-training | Propose a transaction-anomaly + dispute-outcome training run | Director / Data scientist |
...
Figure 1. The platform’s Semantic Models surface (models from many installed packs shown). This pack adds: disputes_core.

Figure 2. The Dashboards surface. This pack installs: Dispute_command_center, Regulatory_clock, Chargeback_recovery.
...
The analytics agent answers KPI questions over the governed disputes_core model(s) — read-only, citing the measures it used. From the pack’s own analytics instructions:

| Analytics grounding (from the pack) Answer dispute-program KPI questions from the governed semantic model disputes_core: cardholder_favor_rate, dispute_denial_rate, chargeback_win_rate, recovery_rate, fraud_dispute_share, avg_dispute_age_days (backlog aging), provisional-credit exposure (provisional_credit_issued_count / provisional_credit_due_count), and transaction-side surfaces (cnp_txn_count, recurring_txn_count). Use plain operational language for dispute-operations and compliance staff and always cite the measure names you used. Slice by dispute type, reason code, network, regulatory regime (reg_flag), severity, deadline bucket, filed month, … |
|---|
...
## model-training 4.4. Propose a transaction-anomaly + dispute-outcome model

It turns a plain-language goal into a governed training-run proposal — algorithm, hyperparameters, label/features — grounded in the algorithm catalog and prior runs. The pack ships a real transaction-anomaly + dispute-outcome training pipeline.

- Ask “train a transaction-anomaly + dispute-outcome model on the panel.” It returns a pipeline.template.create_from_algorithm proposal; approve → the pipeline runs and registers a model.
...
Each queued case (dispute_queue) gets a proposed disposition, grounded in the domain rules and citing the exact basis. It never takes the action itself. From the pack’s own triage instructions:

| Triage grounding (from the pack) You triage US card-issuer disputes under Reg E (12 CFR 1005, debit) and Reg Z (12 CFR 1026.13, credit) plus Visa/Mastercard chargeback rules. Ground every recommendation in the specific transaction evidence — cite transaction ids, amounts, dates, merchants, entry modes, and the cardholder's history (prior disputes, denials, tenure, risk tier). Watch the regulatory clock first: flag any dispute whose deadline_days_left puts a provisional-credit or investigation deadline at risk, and never let an investigation-quality concern justify blowing a Reg E deadline — provisional credit is issued on time and reversed later with proper notice if the investigation finds no error. Propose one disposition per dispute: resolve_cardholder_favor (evidence confirms an error or unauthorized transaction), deny_no_error_found (documented evidence contradicts the claim — a denial requires specific findings th… |
|---|
...
|---|---|---|
| resolve_cardholder_favor | Resolve in cardholder's favor — error confirmed | true_positive |
| deny_no_error_found | Deny — investigation found no error | false_positive |
| file_chargeback | File chargeback — recover from merchant | other |
| escalate_fraud_review | Escalate to fraud investigation (first-party watch) | inconclusive |
| close_merchant_credited | Close — merchant already credited | benign |
...
- Rules as grounding. The domain’s regulatory rules are retrieved as grounding the agents must cite (reg_e, reg_z, cfpb, visa_rules, mastercard_rules).
...
| Capability | Status |
|---|---|
| Semantic models (disputes_core), dashboards, saved/verified queries | Live |
...
| transaction-anomaly + dispute-outcome training pipeline | Live |
