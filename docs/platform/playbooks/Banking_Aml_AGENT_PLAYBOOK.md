<!-- converted from Banking_Aml_AGENT_PLAYBOOK.docx by tools/docs/docx_to_md.py -->
...
DATACERN AI · BANKING AML

Agent Playbook

How to put the nine Datacern agents to work in banking aml.

This playbook maps each of Datacern’s nine agents to a concrete job in this vertical, grounded in the real banking-aml pack.

Pack: banking-aml v1.0.0

Domain: banking · aml · financial-crime · sanctions

Regulatory: bsa, fincen_sar, ofac, sr_11_7, fatf
...
AI-assisted AML / BSA financial-crime workflow for US banks (BRD 30): alert triage with typology-grounded dispositions, sanctions match adjudication, SAR-recommendation queues with MLRO approval, AML KPI semantic model and dashboards (false-positive rate, SAR conversion, backlog aging), party relationship-network analytics, BSA/OFAC grounding memories, and transaction-anomaly + alert-scoring training pipelines.
...
| The rule that shapes everything Datacern proposes; a human decides. AML_AUTONOMOUS_FILING_FORBIDDEN: no agent takes the consequential action on its own — it drafts and recommends; a human on the team approves (four-eyes). Enforced by the platform. |
|---|
...
| model-training | Propose a transaction-anomaly + alert-scoring training run | Director / Data scientist |
...
Figure 1. The platform’s Semantic Models surface (models from many installed packs shown). This pack adds: aml_core.

Figure 2. The Dashboards surface. This pack installs: Aml_command_center, Sanctions_screening, Transaction_monitoring.
...
The analytics agent answers KPI questions over the governed aml_core model(s) — read-only, citing the measures it used. From the pack’s own analytics instructions:

| Analytics grounding (from the pack) Answer AML program KPI questions from the governed semantic model aml_core: alert_false_positive_rate, sar_conversion_rate, avg_alert_age_days (backlog aging / cycle time), sanctions_hit_count, avg_match_score, edd_high_risk_ratio, cash/wire transaction volumes. Use plain operational language for compliance staff and always cite the measure names you used. Slice by typology, severity, assigned tier, alert month, channel, counterparty jurisdiction, or party risk rating. These figures support BSA program oversight and regulator exam prep — never speculate beyond the governed data, and never reference SAR status for… |
|---|
...
## model-training 4.4. Propose a transaction-anomaly + alert-scoring model

It turns a plain-language goal into a governed training-run proposal — algorithm, hyperparameters, label/features — grounded in the algorithm catalog and prior runs. The pack ships a real transaction-anomaly + alert-scoring training pipeline.

- Ask “train a transaction-anomaly + alert-scoring model on the panel.” It returns a pipeline.template.create_from_algorithm proposal; approve → the pipeline runs and registers a model.
...
Each queued case (alert_queue) gets a proposed disposition, grounded in the domain rules and citing the exact basis. It never takes the action itself. From the pack’s own triage instructions:

| Triage grounding (from the pack) You triage US banking AML alerts (BSA/FinCEN regime): structuring, layering, sanctions near-matches, dormant-account reactivation, funnel accounts, and peer-group anomalies. Ground every recommendation in the specific transaction evidence for the typology — cite transaction ids, amounts, dates, counterparties, and the scenario rule that fired — plus the customer's declared business type and risk rating (a cash-intensive SIC code like a restaurant explains cash deposits; a florist does not explain weekly $50K international wires). Propose one disposition per alert: clear_false_positive (evidence contradicts the typology), escalate_l2 (needs deeper case-building: network expansion, adverse media, prior SAR history), or recommend_sar (typology evidence is strong and specific). You NEVER file a SAR and never treat recommend_sar as a filing — the SAR filing decision belongs exclusively to the… |
|---|
...
|---|---|---|
| clear_false_positive | Clear as false positive | false_positive |
| escalate_l2 | Escalate to L2 investigation | inconclusive |
| recommend_sar | Recommend SAR (MLRO decides filing) | true_positive |
| sanctions_true_hit | Sanctions true hit — block & report | true_positive |
| edd_required | EDD required — enhanced due diligence | other |
...
- Rules as grounding. The domain’s regulatory rules are retrieved as grounding the agents must cite (bsa, fincen_sar, ofac, sr_11_7, fatf).
...
| Capability | Status |
|---|---|
| Semantic models (aml_core), dashboards, saved/verified queries | Live |
...
| transaction-anomaly + alert-scoring training pipeline | Live |
