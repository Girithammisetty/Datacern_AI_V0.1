<!-- converted from Construction_Claims_AGENT_PLAYBOOK.docx by tools/docs/docx_to_md.py -->
...
DATACERN AI · CONSTRUCTION CLAIMS

Agent Playbook

How to put the nine Datacern agents to work in construction claims.

This playbook maps each of Datacern’s nine agents to a concrete job in this vertical, grounded in the real construction-claims pack.

Pack: construction-claims v1.0.0

Domain: construction · claims · change_orders · disputes · infrastructure
...
AI-assisted construction claim and change-order adjudication for owners, GCs, CMs, sureties, and infrastructure agencies: claim intake triage with contractual notice-provision awareness, entitlement-first analysis (differing site conditions Type I/II, directed vs constructive acceleration, concurrent delay), quantum review grounded in measured-mile vs total-cost methodology, defect-backcharge and surety-notice handling, pay-if-paid/pay-when-paid payment-dispute screening, a claims-operations KPI semantic model and dashboards (approval/rejection/negotiated rates, claimed-vs-approved ratio, deadline runway, schedule-impact mix), a party-to-project exposure network, claims-law grounding memories, and contract-anomaly + claim-outcome training pipelines.
...
|---|---|
| Claims Analyst | — |
| Scheduling & Delay Specialist | — |
| Contract Administrator | — |
| Claims Review Board Manager | — |
| Project Controls Auditor | — |
...
| model-training | Propose a contract-anomaly + claim-outcome training run | Director / Data scientist |
...
Figure 1. The platform’s Semantic Models surface (models from many installed packs shown). This pack adds: construction_claims_core.

Figure 2. The Dashboards surface. This pack installs: Claims_command_center, Entitlement_schedule_impact, Party_risk_recovery.
...
The analytics agent answers KPI questions over the governed construction_claims_core model(s) — read-only, citing the measures it used. From the pack’s own analytics instructions:

| Analytics grounding (from the pack) Answer claims-program KPI questions from the governed semantic model construction_claims_core: approval_rate, rejection_rate, negotiated_share, approved_to_claimed_ratio (quantum discipline), delay_claim_share, avg_claim_age_days (backlog aging), open backlog and deadline runway (open_claim_count by deadline_bucket), notice hygiene (late_notice_count), schedule exposure (major_schedule_impact_count), and party-book surfaces (subcontractor_count, surety_backed_party_count, repeat_claimant_party_count). Use plain operational language for project-controls, executive, and surety/lender audiences and always cite the m… |
|---|
...
## model-training 4.4. Propose a contract-anomaly + claim-outcome model

It turns a plain-language goal into a governed training-run proposal — algorithm, hyperparameters, label/features — grounded in the algorithm catalog and prior runs. The pack ships a real contract-anomaly + claim-outcome training pipeline.

- Ask “train a contract-anomaly + claim-outcome model on the panel.” It returns a pipeline.template.create_from_algorithm proposal; approve → the pipeline runs and registers a model.
...
Each queued case (claims_queue) gets a proposed disposition, grounded in the domain rules and citing the exact basis. It never takes the action itself. From the pack’s own triage instructions:

| Triage grounding (from the pack) You triage construction claims and change-order requests for the paying side of the contract (owner, GC, or CM). Analyze entitlement FIRST, quantum second: identify the governing contract clause (changes, differing site conditions, delay/time-extension, notice, payment) and whether the asserted basis — design change, owner directive, weather, site conditions, subcontractor default, or spec ambiguity — actually engages it. Ground every recommendation in the specific record evidence: cite claim ids, contract ids, party ids, claimed amounts, notice timeliness, and the party's history (prior claims, prior rejections, prequal tier, surety backing). Check the contractual notice provision first — timely written notice is often a condition precedent and strictly enforced in many jurisdictions — and watch days_to_deadline: never let analysis depth blow a contractual response deadline; propose an … |
|---|
...
|---|---|---|
| approve_change_order | Approve — entitlement and quantum substantiated | true_positive |
| reject_no_entitlement | Reject — no contractual entitlement | false_positive |
| negotiate_partial_merit | Negotiate — partial merit, settle at adjusted quantum | other |
| request_substantiation | Request substantiation — quantum or causation unsupported | inconclusive |
| close_withdrawn_resolved | Close — withdrawn or resolved in the field | benign |
...
- Rules as grounding. The domain’s regulatory rules are retrieved as grounding the agents must cite (contract_law, far_dsc_clause, mechanics_lien, surety_bonds, prompt_payment).
...
| Capability | Status |
|---|---|
| Semantic models (construction_claims_core), dashboards, saved/verified queries | Live |
...
| contract-anomaly + claim-outcome training pipeline | Live |
