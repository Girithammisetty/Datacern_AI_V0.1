...
- **012** Authorization decisions call the local OPA sidecar (`POST localhost:8181/v1/data/datacern/authz/allow`) with `{subject, action, resource_urn, tenant}`; OPA reads the Redis `permissions_flat` projection. Target p99 ≤ 10ms. **Never** call rbac-service synchronously in the request path.
...
| 24 | `24_insurance_claims_payer_pack_BRD.md` | `insurance-claims-payer` Capability Pack (Horizon 1 payer vertical — Datacern Claims product) | 4–5 |
...
**Healthcare pack roster:** nine healthcare / life-sciences packs are built — `insurance-claims-payer`, `payer-fwa-siu`, `healthcare-provider-rcm`, `benefits-appeals`, `care-management-medicare`, `pharmacy-benefit-mgmt`, `post-acute-care`, `device-complaints`, `pharmacovigilance`. Each pack's own `pack.yaml` carries its scope and regulatory mapping; per-vertical agent playbooks are in [`../platform/playbooks/`](../platform/playbooks/). (This previously linked a `HEALTHCARE_PACK_ROADMAP.md` that was never written — no such file exists in the repository or its history.)
