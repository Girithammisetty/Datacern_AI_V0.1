...
| **73** batch job orchestration | B1, B2 | **inc1–inc3b DONE (backend + BFF + UI); AC-9 CLOSED.** `BatchJob`/`BatchJobRun` + migration 0005 (RLS + at-most-one-active-run), the 3-phase machine on the existing lease/reaper machinery, idempotent triggering via ingestion-service's internal MCP facade, an event-driven ingestion phase off `ingestion.events.v1` + `dataset.version_created`, phase deadlines, retry-from-the-failed-phase, REST + outbox events, and the `batch-trigger` component. **inc3b** adds the bff-graphql leg (4 queries + 8 mutations, pure passthrough, no DB/consumer/cache — the CI bans stand) and the Batch Jobs page under Data › Pipelines: the jobs list plus, per run, the `trigger → ingestion → pipeline` timeline with each phase's state, the per-binding ingestion outcomes (pinned dataset **version** urn + snapshot id) and the input/output URNs the run recorded — with "no batch jobs yet" and "we could not load them" as visibly different states. 85 new tests (52 backend + 14 bff-graphql + 19 ui-web); orchestrator unit 260 + integration 16, ingestion-service unit 596, bff-graphql 482, ui-web 965, all green. **Only the agent proposal (AC-10 / inc4) is deferred**; realtime push for the run timeline is a poll today (see the BRD). |
