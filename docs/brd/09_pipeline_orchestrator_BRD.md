...
- **PIPE-FR-041 (M)** Node-pool routing per cloud (mined from V1, now config-driven per cell): AWS → nodeAffinity requiring `node.group in [ml]` OR `workload in [processing]`, preferring `processing`; high-memory nodes (request ≥ 15 GB) route to the large pool; Azure → AppArmor `runtime/default` annotations on every pod; GCP → `serviceAccountName = <tenant>-<env>-wl-ksa` + nodeSelector `iam.gke.io/gke-metadata-server-enabled: "true"`. Tenant tier may pin a dedicated node pool (`windrose.io/tenant-pool` taint/toleration pair).
