...
| agent-runtime | **Blocker CLOSED 2026-08-02; replica bump ready to enable** | `RetrainScheduler.tick()` now claims via `claim_due_retrain_watches()` — one transaction, `SELECT … FOR UPDATE SKIP LOCKED` + `UPDATE last_checked_at=now` before releasing (the same claim pattern the outbox relay uses). A second replica polling concurrently skips the claimed rows and cannot double-open a retrain proposal. Unit-verified (`test_retrain_scheduler.py::test_claim_is_replica_safe`); the real multi-connection lock behavior still wants a live-Postgres run before the `replicas: 2` bump is turned on. |
