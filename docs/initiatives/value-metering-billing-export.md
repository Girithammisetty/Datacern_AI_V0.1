...
**Status: pending — implementation next.** Nothing in this initiative has been built; §2 is the design to build against.

### Slice plan

1. **Slice 1 — `governed_decision` meter end-to-end.** Catalog entry + `usage_raw`/rollup migration (§2.2) → agent-runtime payload fields (`decision_latency_ms`, `edit_distance_bucket`, `proposal_kind`; `pack_name` deferred if the pack-origin plumbing isn't ready, nullable is acceptable for slice 1) → ingest mapping entries (§2.1's filter-based `auto_executed_action` included) → rollup verification. Proves emission → catalog → ingest → rollup on the cheapest meter (no cross-service propagation needed).
2. **Slice 2 — `usage_decisions` attribution.** `decision_urns[]` propagation (§2.5, both services) + `usage_decisions` DDL + split-insert logic (§2.4) + `GET /api/v1/decisions/costs` (VMB-FR-011).
3. **Slice 3 — period close + export.** `billing_periods`/`billing_exports` DDL, advisory-lock close job (§2.6), JSONL/CSV export with checksums (§2.7), `billing.export_ready.v1`.
4. **Slice 4 — pusher adapters.** `BillingPusher` port + `UnconfiguredPusher` stubs (§2.8) wired into the close job's post-export step; `GET /api/v1/billing/periods` (VMB-FR-023).

### Test plan

- **Unit:** ingest mapping filter predicates (auto-execute detection mirrors the existing `agent_run.completed` filter test pattern in `ingest/pipeline_test.go`); `decision_latency_ms`/`edit_distance_bucket` computation in `proposals/service.py`; BR-15 cost-split rounding (banker's rounding, residual to first URN) as a pure function, unit-tested the way `budget/window_test.go` tests `CrossedThresholds` in isolation from infra.
- **Integration (real Kafka/Postgres, per repo convention — `usage-service/test/integration/*_test.go` uses Testcontainers today, same pattern for new tests):** end-to-end `governed_decision` ingest through `RefreshRollups`; `usage_decisions` split-insert idempotency (replay a `decision_urns`-bearing `ai.token_usage.v1` twice, assert row count unchanged); close-job advisory-lock contention (two "replicas" racing `pg_try_advisory_lock` for the same tenant/period, assert exactly one `billing_periods` row); export checksum verification (write, re-read, verify SHA-256 matches); `BillingPushNotConfigured` returned and period still reaches `exported` status (AC-6).
- **E2E (`deploy/e2e/driver.py`):** the existing journey already exercises the exact commit point this meter is keyed on — `step_e_grant_and_apply()` (`driver.py:569-…`) approves a proposal via `POST {agent_runtime}/api/v1/proposals/{proposal_id}/decide` (line 606-609) and records `EVID["proposal_id"]`/`EVID["proposal_urn"]` (lines 483, 643). Add a new assertion step immediately after: poll `GET {usage_service}/api/v1/reports/decisions` (or the raw meter if the report endpoint isn't built yet in an early slice) filtered to the approved `proposal_id`'s tenant/agent and assert a `governed_decision` row with `decision=approved` exists within the p95 ≤30s ingest-lag budget (USG-FR-012) — "approve a proposal → meter row exists," matching BRD 67 AC-1 exactly, using infrastructure (tenant, tokens, agent, proposal) the journey already sets up rather than a new fixture.
