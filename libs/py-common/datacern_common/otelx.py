...
def configure_tracing(
    ...
    *,
    ...
    ``enabled`` defaults to the ``DATACERN_OTEL_ENABLED`` env var (off unless set),
    so unit tests never reach the collector. ``endpoint`` defaults to
    ``OTEL_EXPORTER_OTLP_ENDPOINT`` or ``http://localhost:4317``.
    """
    global _CONFIGURED
    if enabled is None:
        enabled = os.getenv("DATACERN_OTEL_ENABLED", "").lower() in ("1", "true", "yes")
    if not enabled:
        return False
    if _CONFIGURED:
        return True
...
@contextmanager
def span(name: str, tracer_name: str = "datacern"):
