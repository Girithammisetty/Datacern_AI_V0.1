#!/usr/bin/env bash
# Restart pack-service in isolation (inc20: tenant-dataset binding at install —
# the no-dummy-data rule). Mirrors restart_chart.sh; reuses the e2e harness
# boot function so the env recipe stays identical to up.sh.
#
# Usage: deploy/local/restart_pack.sh
set -uo pipefail
cd "$(dirname "$0")"
E2E="$(cd ../e2e && pwd)"
...
say "stopping pack-service"
stop_port "$PORT_PACK" pack
say "restarting pack-service (inc20 dataset binding)"
start_pack
ok "pack-service restarted"
