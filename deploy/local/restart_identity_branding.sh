#!/usr/bin/env bash
# Restart identity-service in isolation to pick up BRD 59 WS3 (white-label
# branding: tenant_branding migration + MinIO logo store + new routes).
# Mirrors restart_chart.sh / restart_tool_plane.sh.
#
# Usage: deploy/local/restart_identity_branding.sh
set -uo pipefail
cd "$(dirname "$0")"
E2E="$(cd ../e2e && pwd)"
...
say "restarting identity-service (BRD 59 WS3 branding)"
start_identity
