...
- **ConfigMap `datacern-config`** — non-secret settings (`.Values.config`), via `envFrom`.
- **Secret `datacern-secrets`** — synced from the cloud secret manager (External
...
helm upgrade --install datacern deploy/helm/datacern \
  -f deploy/helm/datacern/values-aws.yaml \      # or -gcp / -azure
...
```sh
helm lint deploy/helm/datacern -f deploy/helm/datacern/values-aws.yaml
helm template datacern deploy/helm/datacern -f deploy/helm/datacern/values-aws.yaml | less
```
...
- **Python services** (`migrate: true`) get a `pre-install,pre-upgrade` Job that
  runs `python -m alembic upgrade head` against `datacern-secrets`' DB creds.
...
`externalSecrets.remoteKeyPrefix` (default `datacern/prod`). Enable
`externalSecrets.enabled: true` (already set in each `values-<cloud>.yaml`) to
have the operator materialize `datacern-secrets`. Nothing secret is committed.
...
- **Ingress pinned to in-cluster peers.** The chart renders one default-deny
  ...
  allowing ingress on the service port only from other datacern pods. This is the
